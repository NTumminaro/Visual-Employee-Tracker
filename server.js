...
function viewEmployees() {
  db.query(
    'SELECT e.id, e.first_name, e.last_name, r.title, r.salary, CONCAT(e2.first_name, " ", e2.last_name) AS manager FROM employee AS e INNER JOIN roles as r ON e.roles_id = r.id INNER JOIN employee AS e2 ON e.manager_id = e2.id',
...
function viewRoles() {
  db.query(
    "SELECT r.id, r.title, d.dept_name FROM roles AS r INNER JOIN department AS d ON r.department_id = d.id",
    function (err, results) {
      console.table(results);
      start();
    }
  ...
  inquirer
    ...
    .then((response) => {
      db.query(
        ...
        { dept_name: response.department_name },
        function (err, results) {
  ...
  inquirer
    .prompt([
      ...
      },
      {
        type: "input",
        message:
          "What is the id of the new employee's manager? (can be left blank)",
        name: "manager",
      },
    ])
    .then((response) => {
  ...
  inquirer
    .prompt([
      ...
      // }
      {
        type: "input",
        message:
          "Enter the employee id of the employee you would like to modify:",
        name: "id",
      },
      {
        type: "input",
